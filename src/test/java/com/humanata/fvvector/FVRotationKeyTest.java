...
public class FVRotationKeyTest {
	...
	public void testFVRotationKey() throws Exception {
		FVParameters ps = FVParameters.FVParamsN1024S128insecure; // needs zero noise for test to work
		...
		for(int k = 0; k < rk.numberOfKeys(); k++)
		{
			...
			for(int i = 0; i < rk.elementsPerKey(); i++)
			{
				UnivariatePolynomialZp64 sum = 
							ps.ctPolyField.add(rk.getFirstRekeyingPolynomials(k).get(i),
									ps.ctPolyField.multiply(rk.getSecondRekeyingPolynomials(k).get(i), pk.key())
							);
